...
const saveEvent = async () => {
   ...
   const response = await axios.post('http://localhost:3000/api/auth/events', eventData);
console.log(response.data.event._id);
const id= response.data.event._id

    const getresponse =await axios.post('http://localhost:3000/api/auth/getevents', id)
}
...
const updatedEvents = events.filter((event) => event._id !== selectEvent._id);
